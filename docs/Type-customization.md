The same fluent API used to specify [how to build an anonymous variable](Create-and-Build#Build) can be used to instruct AutoFixture how to create every instance of the same type.
...
This approach works pretty well with simple scenarios (with simple or no dependencies). To handle more complex scenarios, it's better to use [relays](Relays).
